...
interface Notification {
   ...
   created_at: string;
   actor?: { name: string };
   actor_id_populated?: { name: string };
}
...
export default function Navbar() {
   ...
   const getColorClasses = (color: string) => {
      switch (color) {
         case 'pink': return 'hover:text-[#FF2BCD]';
         case 'blue': return 'hover:text-[#32F5FF]';
         case 'purple': return 'hover:text-[#8A2FFF]';
         default: return 'hover:text-[#FF2BCD]';
      }
   };
   ...
   const getActiveBorder = (to: string, color: string) => {
                                             ...
                                             <div className="flex items-start space-x-3">
                                                <div className="relative">
                                                   <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FF2BCD] to-[#8A2FFF] flex items-center justify-center text-white font-bold text-sm">
                                                      {(notif.actor?.name?.[0] || notif.actor_id_populated?.name?.[0] || 'U').toUpperCase()}
                                                   </div>
                                                ...
                                                <div className="flex-1 min-w-0">
                                                   <p className="text-sm text-gray-200 leading-tight">
                                                      <span className="font-bold text-white">{notif.actor?.name || notif.actor_id_populated?.name || 'Someone'}</span>

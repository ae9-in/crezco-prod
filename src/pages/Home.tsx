...
import { ArrowRight, Star, Zap, Target, TrendingUp, Globe, Users, Calendar } from 'lucide-react';
...
export default function Home() {
   ...
   return (
      <div className="min-h-screen bg-[#05070A] text-white selection:bg-[#FF2BCD]/30">
         ...
         <section className="relative min-h-[90vh] flex items-center justify-center pt-20 overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
               <div className="space-y-10">
                  <div className="inline-flex items-center space-x-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-md animate-in fade-in slide-in-from-bottom-4 duration-1000">
                  ...
                  <div className="max-w-4xl mx-auto">
                     <h1 className="text-4xl sm:text-6xl md:text-7xl font-black text-white leading-tight tracking-tight mb-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200">
                        Together, We Grow <br />
                     ...
                     <p className="text-lg sm:text-xl md:text-2xl text-gray-400 max-w-2xl mx-auto leading-relaxed mb-12 animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-300">
         ...
         <div className="bg-[#05070A] py-6 border-b border-white/5 overflow-hidden whitespace-nowrap">
            <div className="inline-block animate-marquee">
               {Array.from({ length: 2 }).map((_, i) => (
                  <span key={i} className="inline-flex items-center text-sm font-bold text-gray-500 uppercase tracking-[0.2em]">
                     <span className="mx-8">• {metrics.colleges} colleges joined</span>
                     <span className="mx-8">• {metrics.events} events hosted</span>
                     <span className="mx-8">• {metrics.users} students active</span>
                     <span className="mx-8">• join the revolution</span>
                     <span className="mx-8">• expand your reach</span>
                  </span>
               ))}
            </div>
         </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
                  {user ? (
                     feedLoading ? (
                        ...
                        feed.map((item) => (
                           <div key={item.feed_id} className="group bg-[#0D0F1A] border border-white/5 rounded-[2.5rem] overflow-hidden transition-all hover:-translate-y-2">
                              <div className="p-6 border-b border-white/5 flex items-center space-x-3">
                                 ...
                                 <div className="flex-1 min-w-0">
                                    <p className="text-sm font-bold text-white truncate">{(item as any).author?.name || 'Anonymous'}</p>
                                    <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">{(item as any).collegeName || 'Global Community'}</p>
                                 </div>
                              </div>
                              ...
                              <div className="p-6">
                                 ...
                                 {item.type === 'reel' && (
                                    <div className="relative aspect-[9/16] rounded-2xl overflow-hidden bg-black group-hover:shadow-[0_0_30px_rgba(255,43,205,0.2)] transition-shadow">
                                       <video
                                       ...
                                       />
                                       <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-black text-[#FF2BCD] border border-[#FF2BCD]/30">REEL</div>
                                    </div>
                                 )}
                              </div>
                              ...
                              <div className="px-6 py-4 bg-white/5 flex items-center justify-between">
                                 ...
                                 <span className="text-[10px] font-black uppercase text-[#8A2FFF] tracking-tighter">View post</span>
                              </div>
                           </div>
                        ))
                     ) : (
                     ...
                     // Marketing preview for logged out users
                     Array.from({ length: 3 }).map((_, i) => (
         ...
         <section className="relative py-32 overflow-hidden">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
               <div className="text-center mb-16">
                  <span className="text-[#32F5FF] font-black text-xs uppercase tracking-[0.4em]">Our Mission</span>
                  <h2 className="text-4xl sm:text-6xl font-black mt-4 mb-8 italic">Curiosity Unbound.</h2>
                  <div className="w-20 h-2 bg-gradient-to-r from-[#FF2BCD] to-[#32F5FF] mx-auto rounded-full"></div>
               </div>

               <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                  {[
                     { title: 'Discover', color: '#FF2BCD', icon: '1', text: 'Find your true curiosity and passion in a world of endless possibilities.' },
                     { title: 'Develop', color: '#32F5FF', icon: '2', text: 'Build real-world skills and capabilities through hands-on experience.' },
                     { title: 'Grow', color: '#8A2FFF', icon: '3', text: 'Unlock high-tier opportunities and succeed alongside your community.' }
                  ].map((step, i) => (
                     <div key={i} className="relative group p-10 bg-white/5 border border-white/5 rounded-[3rem] transition-all hover:bg-white/10">
                        <div
                           className="w-16 h-16 rounded-2xl flex items-center justify-center text-white font-black text-2xl mb-8 group-hover:scale-110 transition-transform"
                           style={{ background: `linear-gradient(135deg, ${step.color}, ${step.color}80)` }}
                        >
                           {step.icon}
                        </div>
                        <h3 className="text-2xl font-black mb-4" style={{ color: step.color }}>{step.title}</h3>
                        <p className="text-gray-400 leading-relaxed">{step.text}</p>
                     </div>
                  ))}
               </div>

               <div className="mt-20 p-12 bg-white/5 backdrop-blur-md rounded-[3rem] border border-white/10 text-center">
                  <p className="text-gray-300 text-xl leading-relaxed italic">
                     "Crezco is more than just a platform; it's a movement. We're bridging the gap between education and opportunity, one student at a time."
                  </p>
                  <div className="mt-8 flex items-center justify-center space-x-4">
                     <div className="w-10 h-10 rounded-full bg-gray-800"></div>
                     <div className="text-left">
                        <p className="text-white font-bold">The Crezco Team</p>
                        <p className="text-xs text-gray-500 uppercase tracking-widest font-black">Join the revolution</p>
                     </div>
                  </div>
               </div>
            </div>

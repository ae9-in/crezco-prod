...
const Referrals: React.FC = () => {
  const [formData, setFormData] = useState<{ [key: string]: { name: string; contact: string } }>({
    hiring: { name: '', contact: '' },
  ...
  const [successMessage, setSuccessMessage] = useState<{ [key: string]: boolean }>({
    hiring: false,
    product: false,
    event: false,
    cc: false
  ...
  const handleInputChange = (type: string, field: 'name' | 'contact', value: string) => {
    setFormData(prev => ({
      ...prev,
      [type]: {
        ...prev[type],
            ...
            <div className="bg-[#0D0F1A] border border-[#FF2BCD]/30 rounded-2xl p-6 hover:border-[#FF2BCD] transition-all duration-300">
              ...
              {successMessage.hiring ? (
                <div className="w-full py-3 bg-green-600 text-white font-bold rounded-lg text-center">
                  Referral submitted (placeholder — Google Form later)
                </div>
              ) : (
                <button 

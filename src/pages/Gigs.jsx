...
import { gigApi } from '../lib/supabase';
import { 
  Briefcase, 
  ChevronRight, 
  Zap, 
...
const Gigs = () => {
  ...
  useEffect(() => {
    const fetchGigs = async () => {
      setLoading(true);
      try {
        const data = await gigApi.getGigs({ category, sortBy, search, type });
        setGigs(data);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await gigApi.applyForGig(selectedGig.id, user.id, formData);
      setSubmitted(true);
      setTimeout(() => {
        closeModal();
      }, 2500);
            ...
            {submitted ? (
              ...
              <div className="space-y-10">
                <div>
                   ...
                   <p className="text-gray-500 font-medium mt-2">Apply for this gig and start your journey.</p>
                </div>
